package com.infinity.infoway.rkuniversity.faculty.faculty_calendar;
...
import com.infinity.infoway.rkuniversity.student.student_calender.StudentEventListAdapter;
...
public class FacultyCalendarActivity extends AppCompatActivity implements GridAdapter.IGridAdapterData, View.OnClickListener{
    ...
    private void init() {
        connectionDetector = new ConnectionDetector(FacultyCalendarActivity.this);
        ...
        mySharedPreferences = new MySharedPreferences(FacultyCalendarActivity.this);
    }
    ...
    public void gridAdapterData(ArrayList<EventModel.Array> eventDetailsArrayList) {
        if (eventDetailsArrayList != null) {
            if (eventDetailsArrayList.size() <= 0) {
                ...
                FacultyEventListAdapter facultyEventListAdapter = new FacultyEventListAdapter(FacultyCalendarActivity.this, eventDetailsArrayList);
                rvEventList.setAdapter(facultyEventListAdapter);
            }
        }
    }
    ...
    private void getCalendarEventsApiCall() {
        if (connectionDetector.isConnectingToInternet()) {
            ...
            ApiImplementer.getCalendarEventsApiImplementer(mySharedPreferences.getEmpInstituteId(), new Callback<ArrayList<EventModel>>() {
                @Override
                public void onResponse(Call<ArrayList<EventModel>> call, Response<ArrayList<EventModel>> response) {
                    ...
                    if (response.isSuccessful() && response.body() != null && response.body().size() > 0) {
                        ...
                        customItsCustom.setSlyCalendarData(FacultyCalendarActivity.this, slyCalendarData, response.body());
                    } else {
                        llDynamicContect.setVisibility(View.GONE);
                ...
                public void onFailure(Call<ArrayList<EventModel>> call, Throwable t) {
                    ...
                    llBg.setVisibility(View.GONE);
                    llContentNoDataFound.setVisibility(View.VISIBLE);
                    Toast.makeText(FacultyCalendarActivity.this, "Request Failed:- " + t.getMessage(), Toast.LENGTH_SHORT).show();
                }
            });
        } else {
            Toast.makeText(FacultyCalendarActivity.this, "No internet connection Please try again later.", Toast.LENGTH_SHORT).show();
        }
