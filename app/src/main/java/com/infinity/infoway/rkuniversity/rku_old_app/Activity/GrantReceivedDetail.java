...
import com.infinity.infoway.rkuniversity.rku_old_app.Pojo.GReceivePojo;
...
//        if (status.compareToIgnoreCase("Pending") == 0) {
        ...
        String url = URLS.Get_Grant_Received_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + "1" + "&status=" + status + "&id=" + id + "";
...
//                DialogUtils.hideProgressDialog();
    ...
    private void initView() {
        ...
        tv_approve.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showDialog(GrantReceivedDetail.this,IntentConstants.APPROVED_STATUS_FOR_APPROVAL,ID);
            ...
            public void onClick(View v) {
                DialogUtils.showDialog4YNo(GrantReceivedDetail.this, "", "Are You Sure To Reject ?", new DialogUtils.DailogCallBackOkButtonClick() {
                    ...
                    public void onDialogOkButtonClicked() {
                        showDialog(GrantReceivedDetail.this, IntentConstants.REJECT_STATUS_FOR_APPROVAL,ID);
...
//        if (app_rej.compareToIgnoreCase("approve")==0)
        ...
        {
            url = URLS.Get_Grant_Received_Approved_or_Reject +"&id="+ID+"&remarks="+reason+ "&user_id="+ mySharedPreferecesRKUOLD.getUserID()+"&ip="+""+"&approve_reject=" + "1" + "";

        }
        else
        {
            url = URLS.Get_Grant_Received_Approved_or_Reject +"&id="+ID+"&remarks="+reason+ "&user_id="+ mySharedPreferecesRKUOLD.getUserID()+"&ip="+""+"&approve_reject=" + "2" + "";
