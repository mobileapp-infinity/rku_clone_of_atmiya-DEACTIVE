...
import com.infinity.infoway.rkuniversity.rku_old_app.Adapter.PatentAwardedAdapter;
...
//                ApprovleaveListingAPICall(PAGE_NUMBER, isChecked);
    ...
    public static void ApprovePatentAwarded(final int page_no, final boolean isChecked)
    {
        ...
        if (isChecked)
        {
            url = URLS.Get_Patent_awarded_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + page_no + "&status=" + "2" + "&id=" + "0" + "";
        } else {

            url = URLS.Get_Patent_awarded_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + page_no + "&status=" + "1" + "&id=" + "0" + "";
        }
        url = url.replace(" ", "%20");
