...
import com.infinity.infoway.rkuniversity.rku_old_app.Pojo.PgScolarsGuidedPojo;
...
//        if (status.compareToIgnoreCase("Pending")==0)
        ...
        String url = URLS.Get_PG_Scholars_Guided_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + "1" + "&status=" + status + "&id=" + id + "";
...
//        if (app_rej.compareToIgnoreCase("approve")==0)
        ...
        {
            url = URLS.Get_PG_Scholars_Guided_Approved_or_Reject +"&id="+ID+"&remarks="+reason+ "&user_id="+ mySharedPreferecesRKUOLD.getUserID()+"&ip="+""+"&approve_reject=" + "1" + "";

        }
        else
        {
            url = URLS.Get_PG_Scholars_Guided_Approved_or_Reject +"&id="+ID+"&remarks="+reason+ "&user_id="+ mySharedPreferecesRKUOLD.getUserID()+"&ip="+""+"&approve_reject=" + "2" + "";
    ...
    private void initView()
    {
        ...
        tv_approve.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showDialog(PGScholarsDetail.this,IntentConstants.APPROVED_STATUS_FOR_APPROVAL,ID);
        ...
        tv_delete.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                DialogUtils.showDialog4YNo(PGScholarsDetail.this, "", "Are You Sure To Reject ?", new DialogUtils.DailogCallBackOkButtonClick()
                {
                    ...
                    {
                        showDialog(PGScholarsDetail.this, IntentConstants.REJECT_STATUS_FOR_APPROVAL,ID);
