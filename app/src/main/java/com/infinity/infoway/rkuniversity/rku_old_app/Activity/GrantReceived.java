...
import com.infinity.infoway.rkuniversity.rku_old_app.Adapter.GrantReceivedAdapter;
...
public class GrantReceived extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_grant_received);
...
//                ApprovleaveListingAPICall(PAGE_NUMBER, isChecked);
    ...
    public static  void GrantReceiveListing(final int page_no, final boolean isChecked)
    {
        ...
        if (isChecked)
        {
            url = URLS.Get_Grant_Received_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + page_no + "&status=" + "2" + "&id=" + "0" + "";
        } else {

            url = URLS.Get_Grant_Received_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + page_no + "&status=" + "1" + "&id=" + "0" + "";
        }
        url = url.replace(" ", "%20");
