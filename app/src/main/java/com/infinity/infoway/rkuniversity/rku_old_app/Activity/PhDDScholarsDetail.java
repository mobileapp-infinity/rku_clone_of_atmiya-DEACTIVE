...
import com.infinity.infoway.rkuniversity.rku_old_app.Pojo.PhdScolarGuidedPojo;
...
//        if (status.compareToIgnoreCase("Pending")==0)
        ...
        String url = URLS.Get_PhD_Scholars_Guided_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + "1" + "&status=" + status + "&id=" + id + "";
...
//        if (app_rej.compareToIgnoreCase("approve")==0)
        if (app_rej.compareToIgnoreCase(IntentConstants.APPROVED_STATUS_FOR_APPROVAL) == 0) {
            url = URLS.Get_PhD_Scholars_Guided_Approved_or_Reject + "&id=" + ID + "&remarks=" + reason + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&ip=" + "" + "&approve_reject=" + "1" + "";

        } else {
            url = URLS.Get_PhD_Scholars_Guided_Approved_or_Reject + "&id=" + ID + "&remarks=" + reason + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&ip=" + "" + "&approve_reject=" + "2" + "";
    ...
    private void initView() {
        ...
        tv_approve.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showDialog(PhDDScholarsDetail.this, IntentConstants.APPROVED_STATUS_FOR_APPROVAL, ID);
            ...
            public void onClick(View v) {
                DialogUtils.showDialog4YNo(PhDDScholarsDetail.this, "", "Are You Sure To Reject ?", new DialogUtils.DailogCallBackOkButtonClick() {
                    ...
                    public void onDialogOkButtonClicked() {
                        showDialog(PhDDScholarsDetail.this, IntentConstants.REJECT_STATUS_FOR_APPROVAL, ID);
