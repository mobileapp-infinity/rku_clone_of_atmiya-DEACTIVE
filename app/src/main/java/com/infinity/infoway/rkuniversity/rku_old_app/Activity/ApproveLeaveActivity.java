...
import com.infinity.infoway.rkuniversity.rku_old_app.Adapter.CancelLeavesAdapter;
import com.infinity.infoway.rkuniversity.rku_old_app.Adapter.ViewAll_LastInOutAdapter;
...
//                ApprovleaveListingAPICall(PAGE_NUMBER, isChecked);
    ...
    public static void ApprovleaveListingAPICall(final int PageNo, final Boolean Isc)
    {
        ...
        if (Isc)
        {
            ...
            url = URLS.Get_leave_approve_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "2" + "";
        }
        else
            {

            // url = URLS.Get_leave_approve_list + "&user_id=" + "1" + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "1" + "";
            url = URLS.Get_leave_approve_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "1" + "";
        }
        url = url.replace(" ","%20");
