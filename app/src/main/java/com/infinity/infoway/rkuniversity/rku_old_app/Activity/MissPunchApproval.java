...
import com.infinity.infoway.rkuniversity.rku_old_app.Adapter.MissPunchapprovalAdapter;
...
//        lvmisspunchapproval.setAdapter(missPunchapprovalAdapter);
    ...
    public static void MissPunchApprovalAPiCall(final int PageNo, final Boolean Isc)
    {
        ...
        if (Isc)
        {
            url = URLS.Get_miss_pucn_approve_list + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "2" + "";
        }
        else
        {

            url = URLS.Get_miss_pucn_approve_list + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "1" + "";
        }
        url = url.replace(" ","%20");
