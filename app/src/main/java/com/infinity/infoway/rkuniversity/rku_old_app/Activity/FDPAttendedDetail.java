...
import com.infinity.infoway.rkuniversity.rku_old_app.Pojo.FDPAttendedPojo;
import com.infinity.infoway.rkuniversity.rku_old_app.Pojo.FdpDocumentPojo;
...
//            if (status.contentEquals("Pending")) {
            if (status.contentEquals(IntentConstants.PENDING_STATUS_FOR_APPROVAL)) {
...
//                                        customBoldTextView.setWidth(LinearLayout.LayoutParams.WRAP_CONTENT);
                                        ...
                                        customBoldTextView.setText("  Download Document  ");
    ...
    private class DownloadFileFromURL extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... f_url) {
            ...
            try {
                ...
                conection.connect();
//                File dir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/Rku/" + "/FDPDocument/");
//                dir.mkdirs();
//
//
...
//                // this will be useful so that you can show a tipical 0-100% progress bar
                ...
                File folder = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/" + FileAndFolderInfo.FOLDER_NAME + "/FDPApproval/");
                folder.mkdirs();

                String actualFileName = FileAndFolderInfo.getCustomeFileName(mySharedPreferecesRKUOLD.getEmpCode(), "FDPApproval",
...
//                OutputStream output = new FileOutputStream("sdcard/Rku/" + "/FDPDocument/" + nameoffile);
...
//
//            File file11 = new File(Environment.getExternalStorageDirectory().getAbsolutePath(), "Rku/" + "/FDPDocument/" + nameoffile);
...
//
            ...
            File file11 = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "/" + FileAndFolderInfo.FOLDER_NAME + "/FDPApproval/" + actualFileName);
...
//        if (status.compareToIgnoreCase("Pending") == 0) {
        ...
        String url = URLS.Get_FDP_Attended_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + "1" + "&status=" + status + "&id=" + id + "";
...
//                DialogUtils.hideProgressDialog();
    ...
    private void initView() {
        ...
        tv_approve.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showDialog(FDPAttendedDetail.this, IntentConstants.APPROVED_STATUS_FOR_APPROVAL, ID);
            }
        });
            ...
            public void onClick(View v) {
                DialogUtils.showDialog4YNo(FDPAttendedDetail.this, "", "Are You Sure To Reject ?", new DialogUtils.DailogCallBackOkButtonClick() {
                    ...
                    public void onDialogOkButtonClicked() {
                        showDialog(FDPAttendedDetail.this, IntentConstants.REJECT_STATUS_FOR_APPROVAL, ID);
...
//        if (app_rej.compareToIgnoreCase("approve") == 0) {
        if (app_rej.compareToIgnoreCase(IntentConstants.APPROVED_STATUS_FOR_APPROVAL) == 0) {
            url = URLS.Get_FDP_Attended_Approved_or_Reject + "&id=" + ID + "&remarks=" + reason + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&ip=" + "" + "&approve_reject=" + "1" + "";

        } else {
            url = URLS.Get_FDP_Attended_Approved_or_Reject + "&id=" + ID + "&remarks=" + reason + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&ip=" + "" + "&approve_reject=" + "2" + "";
