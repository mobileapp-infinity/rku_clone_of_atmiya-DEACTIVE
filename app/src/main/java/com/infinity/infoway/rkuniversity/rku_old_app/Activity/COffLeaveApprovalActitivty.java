...
import com.infinity.infoway.rkuniversity.rku_old_app.Adapter.CoffLeaveAdapter;
...
//                ApprovleaveListingAPICall(PAGE_NUMBER, isChecked);
    ...
    public static void CoffApproval(final int PageNo, final Boolean Isc)
    {
        ...
        if (Isc)
        {
            ...
            url = URLS.compensatory_leave_approve_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "2" + "";
        }
        else
        {

            url = URLS.compensatory_leave_approve_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + PageNo + "&status=" + "1" + "";
        }
        url = url.replace(" ","%20");
