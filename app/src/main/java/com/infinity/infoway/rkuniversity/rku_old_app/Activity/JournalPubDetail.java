...
import com.infinity.infoway.rkuniversity.rku_old_app.Pojo.JournalPojo;
...
//        if (status.compareToIgnoreCase("Pending")==0)
        ...
        String url = URLS.Get_publication_in_journal_list + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&emp_id=" + mySharedPreferecesRKUOLD.getEmpID() + "&RowsPerPage=" + URLS.RowsPerPage + "&PageNumber=" + "1" + "&status=" + status + "&id=" + id + "";
                                    ...
                                    ivfile.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View v) {
                                            ...
                                            if (!marshMallowPermission.checkPermissionForExternalStorage()) {
                                                ...
                                                if (journalPojo.getData().get(0).getDownload() != null &&
                                                        !journalPojo.getData().get(0).getDownload().isEmpty()) {
...
//                // this will be useful so that you can show a tipical 0-100% progress bar
                ...
                File folder = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS) + "/" + FileAndFolderInfo.FOLDER_NAME + "/JournalPubApproval/");
                folder.mkdirs();

                String actualFileName = FileAndFolderInfo.getCustomeFileName(mySharedPreferecesRKUOLD.getEmpCode(), "JournalPubApproval",
...
////
            ...
            File file11 = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "/" + FileAndFolderInfo.FOLDER_NAME + "/JournalPubApproval/" + actualFileName);
...
//                Uri uri = FileProvider.getUriForFile(_context, _context.getPackageName() , file11);
    ...
    private void initView() {
        ...
        tvapprove.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showDialog(JournalPubDetail.this, IntentConstants.APPROVED_STATUS_FOR_APPROVAL, ID);
            ...
            public void onClick(View v) {
                DialogUtils.showDialog4YNo(JournalPubDetail.this, "", "Are You Sure To Reject ?", new DialogUtils.DailogCallBackOkButtonClick() {
                    ...
                    public void onDialogOkButtonClicked() {
                        ...
                        showDialog(JournalPubDetail.this, IntentConstants.REJECT_STATUS_FOR_APPROVAL, ID);
...
//        if (app_rej.compareToIgnoreCase("approve")==0)
        if (app_rej.compareToIgnoreCase(IntentConstants.APPROVED_STATUS_FOR_APPROVAL) == 0) {
            url = URLS.Get_publication_in_journal_Approved_or_Reject + "&id=" + ID + "&remarks=" + reason + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&ip=" + "" + "&approve_reject=" + "1" + "";

        } else {
            url = URLS.Get_publication_in_journal_Approved_or_Reject + "&id=" + ID + "&remarks=" + reason + "&user_id=" + mySharedPreferecesRKUOLD.getUserID() + "&ip=" + "" + "&approve_reject=" + "2" + "";
